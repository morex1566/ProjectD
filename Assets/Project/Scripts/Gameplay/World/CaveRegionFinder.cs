...
namespace TRPG.Runtime
{
    ...
    /// </summary>
    public static class CaveRegionFinder
    {
        ...
        /// <summary>
        /// 지정한 모든 Empty 좌표가 하나의 공간으로 연결되어 있는지 확인합니다.
        /// </summary>
        public static bool AreCoordinatesConnected(WorldMap worldMap, IReadOnlyList<Vector2Int> coordinates)
        {
            ...
            FloodFill(worldMap, coordinates[0], visited, out _);

            for (int coordinateIndex = 1; coordinateIndex < coordinates.Count; coordinateIndex++)
            {
                if (visited.Contains(coordinates[coordinateIndex]) == false)
                {
                    return false;
                }
            }
        ...
        private static void FindChunkCaveRegions(WorldMap worldMap, WorldChunk chunk, HashSet<Vector2Int> visited, List<List<Vector2Int>> caveRegions)
        {
            for (int localY = 0; localY < worldMap.TilesPerChunk; localY++)
            {
                for (int localX = 0; localX < worldMap.TilesPerChunk; localX++)
                {
                    ...
                    Vector2Int worldCoordinate = chunk.Coordinate * worldMap.TilesPerChunk + new Vector2Int(localX, localY);
